...
if(length(BNB_unmatched_indexes) > 0) {
  ...
  target_urls <- lapply(paste("https://www.bl.uk", target_slugs, sep = ""), URLencode) %>% unlist()
  target_filenames <- basename(target_slugs) %>% str_replace("[^.zip]*$","")
  
  download.file(target_urls, destfile = paste("raw data/zipped", target_filenames, sep = "/"), method = "libcurl")
}
